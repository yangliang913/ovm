...
from stackone.core.platforms.kvm.kvm_constants import my_platform
...
class KVMDomain(VM):
    ...
    def _no_share_hot_migrate(self, dest, live, port):
        cfg = self.get_config()
        ...
        cfg_clone.write()
        self.node.get_vmm().no_share_hot_migrate(self.pid, dest, live, port, cfg_clone)
    ...
    def get_vnc_port(self):
        if self._vm_info is not None:
            ...
            if vnc_port_string and vnc_port_string[0] == ':':
                vnc_port = vnc_port_string[1:].split(',')
                return int(vnc_port[0])
