...
function getContextMenu(nodetype,node){//DATA_CENTER,SERVER_POOL,MANAGED_NODE,VM,IMAGE_STORE,IMAGE_GROUP,IMAGE
    ...
    if(nodetype=="DATA_CENTER"){
        ...
        items.push("-");
        items.push({id: 'provision_vm',text: 'Provision VM',icon:'/icons/provision_vm.png'});
        items.push({id: 'restore_from_backup',text: 'Restore VM',icon:'/icons/small_restore.png'});
        items.push({id: 'restore_hibernated',text: 'Restore Hibernated',icon:'/icons/small_restore.png'});
        ...
        items.push({id: 'backup_now',text: 'Backup Now' ,icon:'/icons/file_edit.png',tooltip:'Backup Now'});
            ...
            success: function(xhr) {//alert(xhr.responseText);
                ...
                if(response.success){
                    if(response.rows!=null){
                        ...
                        objContext.hide_purge_backup = true;
                        menu_context.context_type="leftnav";
                        var menu=createMenu(response.rows, objContext);
                        c=menu;
                        if(node.attributes.nodetype=="DOMAIN"){
                            var dom_id=node.attributes.id; 
...
function createMenu(rows, objContext){
    var hide_purge_backup = false;
    var backup_grid = false;
    if(objContext) {
        if(objContext.hide_purge_backup != undefined) {
            if(objContext.hide_purge_backup == true) {
                hide_purge_backup = true;
            }
        } else if(objContext.id != undefined) {
            if(objContext.id == "backuptask_info_grid_b" || objContext.id == "backupresult_info_grid" || objContext.id == "CLOUD_TEMPLATE") {
                backup_grid = true;
    ...
    for(var i=0;i<rows.length;i++){
        if(backup_grid == true) {
            if(rows[i].value == "purge_backup" || rows[i].value == "restore_backup") {
                items.push({id:rows[i].value, text:rows[i].text, tooltip:rows[i].text, icon:"/icons/"+rows[i].icon});
            }else if(rows[i].value=="create_template"){       
                items.push({id:rows[i].value, text:rows[i].text, tooltip:rows[i].text,icon:"/icons/"+rows[i].icon,
                    provider_id:objContext.provider_id,account_id:objContext.account_id,region_id:objContext.region_id,
                    vdc_id:objContext.vdc_id
                });
            ...
            if(hide_purge_backup == true && rows[i].value == "purge_backup") {
                items.pop();
                continue;
            }
            items.push({id: rows[i].value,text: rows[i].text,tooltip:rows[i].text,icon:"/icons/"+rows[i].icon});
