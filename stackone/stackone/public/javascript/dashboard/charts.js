...
function getChartHdrMsg(node,period,type){
    return '<div class="toolbar_hdg" >'+format(_("{0}使用情况"),type)+'<div>';
}
...
//                                period_combo.getValue(),fdate,tdate,chartid,false,chart_panel);
    ...
    var storelst = null;
    var metricComboWidth = 80;
    if (node_type==stackone.constants.VDC || node_type==stackone.constants.CLOUD_VM){
        ...
        if(node.attributes.cp_type == stackone.constants.CMS){
            storelst = [[stackone.constants.CPU,_(stackone.constants.CPU)],
                    ...
                    [stackone.constants.STORAGE,_(stackone.constants.STORAGE)]
                   ];
        } else {
        ...
        metricComboWidth = 120;
    }

    var metric_combo1=getMetricCombo('Metric',metricComboWidth,metric,60,"combo_ct", storelst);
    var metric_combo2=getMetricCombo('Metric',metricComboWidth,metric,60,"combo_ct", storelst);
            ...
            click: function(btn) {
                ...
                if(compare.getValue()){
                    if(ent2_combo.getValue()=='' || ent1_combo.getValue()==''){
                        Ext.MessageBox.alert(_("警告."),_('请选择比较的实体.'));
                        return false;
                    }
                    if(ent1_combo.getValue()==ent2_combo.getValue()){
                        Ext.MessageBox.alert(_("警告."),_('请选择不同的实体.'));
                        return false;
                    }
                    chart_type=stackone.constants.COMPARISONCHART;
                    ...
                    if(ent1_combo.getValue()==''){
                        Ext.MessageBox.alert(_("警告."),_('请选择一个实体.'));
                        return false;
                    }
                }
...
//                    if(tgts[key]!=null && (node_type==stackone.constants.MANAGED_NODE
//                    || node_type==stackone.constants.DOMAIN)){
...
//        },
    ...
    ht=180
    if (node_type===stackone.constants.MANAGED_NODE){
       ht = 210
    }
...
function redrawChart(node_type,metric,node_id,node_name,period,frmdate,todate,
    ...
    if(period===stackone.constants.CUSTOM){        
        if(frmdate == null || todate==null ||
            frmdate == "" || todate==""){
            Ext.MessageBox.alert(_("警告"),_("From and To should be selected for the Custom Period."))
            return;
        }
    }else if(period===stackone.constants.DTD){
...
CustomPeriodUI=function(title,fdate,tdate,period){
    ...
    this.validate=function(){        
        if(!frmtime.validate()){
            Ext.MessageBox.alert(_("警告."),_('Please correct the From Time.'));
            return false;
        }
        if(!totime.validate()){
            Ext.MessageBox.alert(_("警告."),_('Please correct the To Time.'));
            return false;
        }
        if(this.fromValue()==''||this.fromValue()==null){
            Ext.MessageBox.alert(_("警告."),_('From Date can not be empty.'));
            return false;
        }
        if(this.toValue()==''||this.toValue()==null){
            Ext.MessageBox.alert(_("警告."),_('To Date can not be empty.'));
            return false;
        }
        if(this.fromTime()==this.toTime()){
            Ext.MessageBox.alert(_("警告."),_('From and To can not be same.'));
            return false;
        }
        if(this.fromTime()>this.toTime()){
            Ext.MessageBox.alert(_("警告."),_('From can not be greater than To.'));
            return false;
        }
        if((this.toTime()-this.fromTime())<(1*60*60*1000)){
            Ext.MessageBox.alert(_("警告."),_('Difference between From and To can not be less than 1 Hour.'));
            return false;
        }
        return true;
    };
...
function getMetricCombo(label,width,val,lblwdth,ctCls,storelst){
//    alert("bbbb");
    var storelist;
    if(storelst==null){
        storelist = [[stackone.constants.CPU,_('CPU')],[stackone.constants.MEM,_('Memory')]];
    }else{
        storelist=storelst;
    }
    var lbl='';
    var wdth=100,lblwidth=100;;
    var value=stackone.constants.CPU;
    if(label!=null)
        lbl=label;
    if(width!=null)
        wdth=width;
...
function getPeriodCombo(label,width,val,lblwdth,ctCls){
    ...
    var value=stackone.constants.HRS12;
    if(label!=null)
        lbl=label;
    if(width!=null)
        wdth=width;
    ...
    var combo=new Ext.form.ComboBox({
        ...
        ctCls:ctCls,
        labelStyle: 'width:'+lblwidth+'px;',
        store:[[stackone.constants.HRS12,_('最近12小时')],
                [stackone.constants.DTD,_('今天')],
                [stackone.constants.HRS24,_('最近24小时')],
                [stackone.constants.DAYS7,_('最近7天')],
                [stackone.constants.DAYS30,_('最近30天')],
                [stackone.constants.WTD,_('本周至今')],
                [stackone.constants.MTD,_('本月至今')],
                [stackone.constants.CUSTOM,_('自定义时间')]],
        forceSelection: true,
        mode:'local',
        value:value
    });
...
function getTop5Info(){
    var info_button=new Ext.Button({
        ...
        cls: 'x-btn-icon',
        tooltip: {
            text:'基于过去1小时计算的负载最高的服务器。显示的是这些服务器历史数据.'
        }
    });
    return info_button;
}
...
function getPeriodStartDate(period,enddate){
    ...
    //alert(period);
    if(period===stackone.constants.HRS12){
        startdate.setHours(startdate.getHours()-12);
    }else if(period===stackone.constants.HRS24){
        startdate.setHours(startdate.getHours()-24);
    }else if(period===stackone.constants.DAYS7){
        startdate.setDate(startdate.getDate()-7);
    }else if(period===stackone.constants.DAYS30){
        startdate.setDate(startdate.getDate()-30);
    }else if(period===stackone.constants.DTD){
        startdate.setHours(0);
        startdate.setMinutes(0);
    }else if(period===stackone.constants.WTD){
        startdate.setDate(startdate.getDate()-startdate.getDay());
        startdate.setHours(0);
        startdate.setMinutes(0);
    }else if(period===stackone.constants.MTD){
        startdate.setDate(1);
        startdate.setHours(0);
        startdate.setMinutes(0);
    }
