...
#    "${script_path}"/exp_scp "${source}" "${user_name}" "${server}" "${destination}" "${password}"
    if [ -e "${script_path}"/exp_scp.py ]; then
        echo $pass | python "${script_path}"/exp_scp.py "${source}" "${user_name}" "${server}" "${destination}"
    else
        echo $pass | python "${script_path}"/exp_scp.pyc "${source}" "${user_name}" "${server}" "${destination}"
    fi
else
