...
function remove_mount
{
    ...
    sed -i "s|${device} ${mountpoint} ${storage_type} defaults||" /etc/fstab
}
...
mountpoint=$value
parse_field_values $mountParams 'device'
device=$value

check_prerequisite '' 'df'

echo "DateTime: $(date)" >>$logfilename
echo "$command -t $storage_type $device $mount_point" >>$logfilename
...
case "$storage_type" in
  ocfs2 )  
    case "$command" in
        #grep -w ${device} |
      GET_DISKS | GET_DISKS_SUMMARY ) output=`df --block-size=G | grep -v "Used" |  awk '{ if (NF == 1)  printf("%s ", $0); else print;}' | awk '{ if ($6 == "'${mountpoint}'") print; }' |  awk '{sub(/G/,"",$2); sub(/G/,"",$3); sub(/G/,"",$4); print "OUTPUT=OUTPUT|FILESYSTEM="$1"|SIZE="$2"|USED="$3"|AVAILABLE="$4"|MOUNT="$6""}'`
...
#                     echo "ERROR:server name or mount point is incorrect"
      ...
      #GET_DISKS_SUMMARY)
                  total=0
                  output=`df --block-size=G | grep -v "Used" |  awk '{ if (NF == 1)  printf("%s ", $0); else print;}' | awk '{ if ($6 == "'${mountpoint}'") print; }' |  awk '{sub(/G/,"",$2); sub(/G/,"",$3); sub(/G/,"",$4); print "SUMMARY=SUMMARY|TOTAL="$2""}'`
                  echo $output
                  ;;
       ...
       ATTACH ) #check whether mountpoint exists or not, if not then mount storage.
                output=`mount | awk '{if ($1 == "'${device}'" && $3 == "'${mountpoint}'") print;}'`
                if [ "${output}" == "" ] ; then
                    echo "Mounting ocfs storage..."
                    mount -t "${storage_type}" "${device}" "${mountpoint}"

                    #check mountpoint entry in fstab
                    output=`grep "'${device}'" /etc/fstab | awk '{if($2 == "'${mountpoint}'") print};'`
                    if [ "${output}" == "" ] ; then
                        #add to fstab
                        echo "Adding entry to fstab..."
                        echo "${device} ${mountpoint} ${storage_type} defaults" >> /etc/fstab
                    fi
                else
                    echo "ocfs storage already mounted..."
                fi
                ;;
        CREATE )
        ...
        REMOVE )
            ...
            ;;
      * )
          echo "Usage: -t{ocfs2} -o{GET_DISKS|DETACH|ATTACH}"
          exit 1
          ;;
  esac
esac
