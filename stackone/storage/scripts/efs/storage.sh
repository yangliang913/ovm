...
mountpoint=$value
parse_field_values $mountParams 'volume'
share=$value

#echo  "$server	$domain">>/etc/hosts
      ...
      #GET_DISKS_SUMMARY)
       ...
       ATTACH ) #check whether mountpoint exists or not, if not then mount storage.
                ...
                if [ "${output}" == "" ] ; then
                    ...
                    if [ "${output}" == "" ] ; then
                        echo "Mounting glusterfs storage..."
                        mount -t ${storage_type} ${server}:${share} ${mountpoint}
			echo "Mounting glusterfs storage... ${storage_type} ${server}:${share} ${mountpoint}"
                        ...
                        echo "glusterfs storage already mounted..."
                    fi
                fi
                  ;;
        CREATE )
        ...
        REMOVE )
            ...
            ;;
      * )
          echo "Usage: -t{glusterfs} -o{GET_DISKS|DETACH|ATTACH}"
          exit 1
          ;;
  esac
esac
